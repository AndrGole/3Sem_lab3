...
BOOST_AUTO_TEST_CASE(BenchmarkInsertSorted) {
    const size_t SIZES[] = {100, 1000, 10000, 50000};

    for (size_t size : SIZES) {
        std::vector<int> keys(size);
        std::iota(keys.begin(), keys.end(), 1); // 1, 2, 3, ..., size
        ...
        BOOST_TEST_MESSAGE("Insert " << size << " sorted elements: "
                          << duration.count() << " ms");
        BOOST_CHECK_EQUAL(tree.size(), size);
        BOOST_CHECK(tree.isBalanced());
        BOOST_CHECK(tree.validate());
...
BOOST_AUTO_TEST_CASE(BenchmarkAVLBalance) {
    // Сравнение с несбалансированным BST
    const size_t SIZE = 10000;

    // Случайные данные (хорошо для AVL)
    auto randomKeys = generateUniqueKeys(SIZE);

    // Отсортированные данные (худший случай для обычного BST, но хороший для AVL)
    std::vector<int> sortedKeys(SIZE);
    std::iota(sortedKeys.begin(), sortedKeys.end(), 1);

    // AVL с случайными данными
    auto startAVLRandom = std::chrono::high_resolution_clock::now();
    AVLTree avlRandom;
    for (int key : randomKeys) {
        avlRandom.insert(key);
    }
    auto endAVLRandom = std::chrono::high_resolution_clock::now();
    auto timeAVLRandom = std::chrono::duration_cast<std::chrono::milliseconds>(endAVLRandom - startAVLRandom);

    // AVL с отсортированными данными
    auto startAVLSorted = std::chrono::high_resolution_clock::now();
    AVLTree avlSorted;
    for (int key : sortedKeys) {
        avlSorted.insert(key);
    }
    auto endAVLSorted = std::chrono::high_resolution_clock::now();
    auto timeAVLSorted = std::chrono::duration_cast<std::chrono::milliseconds>(endAVLSorted - startAVLSorted);

    BOOST_TEST_MESSAGE("AVL Tree performance comparison:");
    BOOST_TEST_MESSAGE("  Random data insertion (" << SIZE << " elements): "
                      << timeAVLRandom.count() << " ms, height: " << avlRandom.getHeight());
    BOOST_TEST_MESSAGE("  Sorted data insertion (" << SIZE << " elements): "
                      << timeAVLSorted.count() << " ms, height: " << avlSorted.getHeight());

    // Высота AVL дерева должна быть O(log n)
    size_t log2n = static_cast<size_t>(std::log2(SIZE)) + 1;
    BOOST_CHECK(avlRandom.getHeight() <= 2 * log2n); // AVL гарантирует высоту <= 1.44*log2(n)
    BOOST_CHECK(avlSorted.getHeight() <= 2 * log2n);

    BOOST_TEST_MESSAGE("  Expected max height (1.44*log2(" << SIZE << ")): ~"
                      << static_cast<int>(1.44 * std::log2(SIZE)));
}
